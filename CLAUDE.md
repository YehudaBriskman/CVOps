...
- **Bot identity.** Claude-authored commits end with `Generated by [<Agent-Role>-Agent] [Claude-Bot] of [@me]` on the final line (blank line before it). `@me` is intentionally generic — it stands for whichever contributor is running the work, so the line reads the same for everyone.
- **Pull requests.** Open PRs with the repo helper `scripts/open-pr.sh` (alias `git open-pr`), never hand-rolled `gh pr create` — it enforces these conventions and writes the body in the house format (a why-focused intro, a `## Changes` list, and `Closes #N` lines when the work resolves issues). PR (and commit) bodies must **not** contain external/vendor URLs (no "Generated with Claude Code", `claude.com`/`anthropic.com`, or any tool-attribution link); the only sign-off is `— Claude-Bot on behalf of @me`. Feature branches PR into `dev`; promote `dev → main` with `git open-pr -p` (never push straight to a protected branch).
