...
- **Routers without prefixes** (`data_sources`, `samples`, `ontologies`, `datasets`, `workflows`, `runs`, `models`, `training_containers`) define their full paths inline (e.g. `/projects/{id}/samples`). `auth`, `orgs`, `projects`, `registry`, `internal` are prefixed **in `main.py`** — the router objects themselves must NOT also declare `APIRouter(prefix=…)`, or paths double up (e.g. `/projects/projects`).
